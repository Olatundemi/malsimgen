...
age02 <- user(integer=TRUE)#Just created
...
#  #Slide positivity in ages upto 2 and upto 10
dim(prev0to2) <- c(age02, nh)
dim(prev0to10) <- c(age10, nh)

prev0to2[1:age02,] <- T[i,j] + D[i,j]  + A[i,j]*p_det[i,j]
prev0to10[1:age10,] <- T[i,j] + D[i,j]  + A[i,j]*p_det[i,j]

prev2to10 <- sum(prev0to10[,]) - sum(prev0to2[,])
total_den2to10 <- sum(den[1:age10]) - sum(den[1:age02])

output(prev_out_2to10) <- prev2to10/total_den2to10
...
#clinical incidence in 2-10

dim(clin_inc0to2) <- c(age02,nh) #0-before 2years
clin_inc0to2[1:age02,] <- clin_inc[i,j]

dim(clin_inc0to10) <- c(age10,nh) #0-10years
clin_inc0to10[1:age10,] <- clin_inc[i,j]

total_2to10 = sum(clin_inc0to10)-sum(clin_inc0to2)
output(inc2_10) <- total_2to10/total_den2to10

#clinical incidence in all
